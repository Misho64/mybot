...
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : '▢ *Dawnload By:* Bropilt Bot';
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message:
          "حدث خطأ أثناء الحصول على معرف تويتر.  تأكد من صحة عنوان URL الخاص بتويتر!",
      });
